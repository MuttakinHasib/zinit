...
echo ">>> Downloading muttakinhasib/zplugin module to $ZINIT_HOME/mod-bin"
if test -d "$ZINIT_HOME/mod-bin/.git"; then
    cd "$ZINIT_HOME/mod-bin"
    git pull origin master
else
    cd "$ZINIT_HOME"
    git clone --depth 10 https://github.com/muttakinhasib/zplugin.git mod-bin
fi
echo ">>> Done"
...
echo "$col_pname== Building module muttakinhasib/zplugin, running: a make clean, then ./configure and then make ==$col_rst"
...
CPPFLAGS=-I/usr/local/include CFLAGS="-g -Wall -O3" LDFLAGS=-L/usr/local/lib ./configure --disable-gdbm --without-tcsetpgrp && {
  ...
  make && {
    ...
    echo "    zmodload muttakinhasib/zplugin"
    echo ""
    ...
    echo "while the module is loaded (check with Zsh command \`zmodload')."
      ...
      echo "error-report at: https://github.com/muttakinhasib/zinit/issues"
  }
}
