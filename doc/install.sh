...
if command -v curl >/dev/null 2>&1; then
    mkdir -p /tmp/zinit
    cd /tmp/zinit 
    curl -fsSLO https://raw.githubusercontent.com/muttakinhasib/zinit/master/git-process-output.zsh && \
        chmod a+x /tmp/zinit/git-process-output.zsh
elif command -v wget >/dev/null 2>&1; then
    mkdir -p /tmp/zinit
    cd /tmp/zinit 
    wget -q https://raw.githubusercontent.com/muttakinhasib/zinit/master/git-process-output.zsh && \
        chmod a+x /tmp/zinit/git-process-output.zsh
fi
...
if test -d "$ZINIT_HOME/$ZINIT_BIN_DIR_NAME/.git"; then
    ...
    { git clone --progress https://github.com/muttakinhasib/zinit.git "$ZINIT_BIN_DIR_NAME" \
...
if [[ ! -f $ZINIT_HOME/$ZINIT_BIN_DIR_NAME/zinit.zsh ]]; then
    print -P "%F{33}▓▒░ %F{220}Installing %F{33}DHARMA%F{220} Initiative Plugin Manager (%F{33}muttakinhasib/zinit%F{220})…%f"
    command mkdir -p "$ZINIT_HOME" && command chmod g-rwX "$ZINIT_HOME"
    command git clone https://github.com/muttakinhasib/zinit "$ZINIT_HOME/$ZINIT_BIN_DIR_NAME" && \\
...
- [38;5;226mREADME[0m section on the ice-modifiers:
    - https://github.com/muttakinhasib/zinit#ice-modifiers,
- [38;5;226mintro[0m to Zinit at the Wiki:
    - https://muttakinhasib.org/zinit/wiki/INTRODUCTION/,
...
- [38;5;226mFor-Syntax[0m article on the Wiki; it is less directly related to the ices, however, it explains how to use them conveniently:
    - https://muttakinhasib.org/zinit/wiki/For-Syntax/.
EOF
