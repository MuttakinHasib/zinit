...
.zinit-module() {
    if [[ "$1" = "build" ]]; then
        ...
        if [[ "$2" = "--link" ]]; then
              ...
              builtin print -r "error-report at: https://github.com/muttakinhasib/zinit/issues"
        else
            ...
            builtin print -r "    zmodload muttakinhasib/zplugin"
            builtin print -r ""
...
.zinit-build-module() {
    ...
    ( builtin cd -q "${ZINIT[BIN_DIR]}"/zmodules
      +zinit-message "{pname}== Building module muttakinhasib/zplugin, running: make clean, then ./configure and then make =={rst}"
      ...
      CPPFLAGS=-I/usr/local/include CFLAGS="-g -Wall -O3" LDFLAGS=-L/usr/local/lib ./configure --disable-gdbm --without-tcsetpgrp && {
          ...
          if { make } {
            [[ -f Src/muttakinhasib/zplugin.so ]] && cp -vf Src/muttakinhasib/zplugin.{so,bundle}
